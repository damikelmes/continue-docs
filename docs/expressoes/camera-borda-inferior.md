---
title: "Borda inferior da câmera"
description: "Retorna a coordenada da extremidade inferior visível."
---

<div class="node-page-kicker" style="--node-page-color: #60A5FA">
  <span class="node-page-meta">
    <span class="node-kind">EXPRESSÃO</span>
    <span class="node-category-name">Câmera</span>
  </span>
</div>

<h1 class="node-page-title" style="--node-page-color: #60A5FA">
  <span class="node-title-icon"><NodeIcon type="plain" icon="arrow-down-outline" color="#60A5FA" :size="26" /></span>
  <span>Borda inferior da câmera</span>
</h1>

<p class="node-purpose">Retorna a coordenada da extremidade inferior visível.</p>

## O que retorna

Retorna a coordenada da extremidade inferior visível.

## Como escrever

<div class="expression-token" style="--expression-color: #60A5FA"><code>camera.bottom</code></div>

Use o botão de procurar expressão no campo numérico para o editor escrever o nome correto automaticamente.

## Exemplos de uso

<div class="expression-example-list" style="--expression-color: #60A5FA">
  <section class="expression-example">
    <strong>1. Ler borda inferior da câmera</strong>
    <pre><code>camera.bottom</code></pre>
    <p>Usa o valor atual de borda inferior da câmera no momento em que o nó é executado.</p>
  </section>
  <section class="expression-example">
    <strong>2. Fixar uma margem inferior</strong>
    <pre><code>camera.bottom - 20</code></pre>
    <p>Posiciona algo 20 pixels acima da borda inferior.</p>
  </section>
  <section class="expression-example">
    <strong>3. Detectar uma queda para fora</strong>
    <pre><code>camera.bottom</code></pre>
    <p>No nó Comparar valores, compare jogador.top com esta expressão usando a opção Maior que.</p>
  </section>
</div>

## Regras

- A expressão usa a câmera atual da cena.
- O valor considera o zoom quando isso altera a área visível.

## Expressões relacionadas

- [Centro X da câmera](./camera-centro-x) — `camera.x`
- [Centro Y da câmera](./camera-centro-y) — `camera.y`
- [Borda esquerda da câmera](./camera-borda-esquerda) — `camera.left`
- [Borda direita da câmera](./camera-borda-direita) — `camera.right`
- [Borda superior da câmera](./camera-borda-superior) — `camera.top`

<a class="node-back-link" href="./">← Ver todas as expressões</a>
